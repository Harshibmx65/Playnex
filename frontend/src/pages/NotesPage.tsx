    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-white/[0.08]">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-bold text-cyan-700 dark:text-[#00e5ff] bg-cyan-50 dark:bg-[#00e5ff]/10 px-3 py-1.5 rounded-xl border border-cyan-200 dark:border-[#00e5ff]/30">
      ...
      {isLoading ? (
        <div className="py-20 text-center">
          ...
          <p className="text-xs font-mono text-slate-500 dark:text-slate-400">Loading notes...</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredNotes.map((note) => (
            <div
              key={note.id}
              className="p-4 rounded-2xl bg-white dark:bg-[#0c1426] border border-slate-200 dark:border-white/[0.08] hover:border-cyan-500/40 transition-all shadow-sm dark:shadow-md flex flex-col justify-between space-y-3 group"
            >
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Link
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-cyan-50 dark:bg-[#00e5ff]/10 hover:bg-cyan-100 dark:hover:bg-[#00e5ff]/20 text-cyan-700 dark:text-[#00e5ff] text-xs font-mono font-bold border border-cyan-200 dark:border-[#00e5ff]/30 transition-colors"
                    ...
                    {note.playlist_title && (
                      <span className="text-[11px] font-mono text-slate-500 dark:text-slate-400 truncate max-w-[200px]">
                        {note.playlist_title}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1 rounded-lg text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/[0.05] transition-colors"
                      title="Edit note"
                    >
                ...
                {editingNoteId === note.id ? (
                  <div className="space-y-2 pt-1">
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-slate-50 dark:bg-[#070b14] border border-slate-300 dark:border-white/10 rounded-lg text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-cyan-500 font-bold"
                    />
                    <textarea
                      value={editContent}
                      onChange={(e) => setEditContent(e.target.value)}
                      rows={3}
                      className="w-full px-3 py-2 bg-slate-50 dark:bg-[#070b14] border border-slate-300 dark:border-white/10 rounded-lg text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-cyan-500"
                    />
