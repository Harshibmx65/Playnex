    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in font-sans">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all ${
              isRevisionFocusMode
                ...
                : 'bg-slate-100 dark:bg-[#0c1426] text-slate-800 dark:text-slate-200 hover:text-slate-950 dark:hover:text-white border border-slate-300 dark:border-white/15'
            }`}
          >
      ...
      {isRevisionFocusMode && (
        <div className="p-5 rounded-3xl bg-gradient-to-r from-amber-500/20 via-amber-500/10 to-transparent border border-amber-500/30 text-xs space-y-2">
          <div className="flex items-center gap-2 font-mono font-black text-amber-900 dark:text-amber-300 text-sm">
            <Flame className="w-4 h-4 text-amber-600 dark:text-amber-400" />
            <span>REVISION DRILL ACTIVE</span>
          </div>
          <p className="text-slate-800 dark:text-slate-200 max-w-xl font-medium">
          ...
          {(['ACTIVE', 'REVISED', 'ALL'] as const).map((st) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                statusFilter === st
                  ? 'bg-white dark:bg-slate-800 text-amber-800 dark:text-amber-300 shadow-sm'
      ...
      {isLoading ? (
        <div className="py-20 text-center">
          ...
          <p className="text-xs font-mono font-bold text-slate-700 dark:text-slate-300">Loading revision queue...</p>
        </div>
        ...
        <div className="py-16 text-center rounded-2xl border-2 border-dashed border-slate-300 dark:border-white/10 bg-white/60 dark:bg-[#0c1426]/50 p-8 space-y-2">
          <RotateCw className="w-10 h-10 text-slate-400 dark:text-slate-500 mx-auto" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredRevisions.map((rev) => {
            ...
            return (
              <div
                key={rev.id}
                className={`p-4 rounded-2xl border-2 transition-all shadow-sm flex flex-col justify-between space-y-3 group ${
                  isRevised
                    ? 'bg-white/80 dark:bg-[#0c1426]/70 border-slate-200 dark:border-white/[0.08] opacity-85'
                    : 'bg-white dark:bg-[#0c1426] border-slate-200 dark:border-white/[0.08] hover:border-amber-500/50'
                }`}
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    <span className={`px-2.5 py-0.5 rounded-full font-mono text-xs font-bold border ${badge.bg}`}>
                  ...
                  >
                    <h3 className="text-sm font-bold text-slate-950 dark:text-white group-hover/link:text-cyan-600 dark:group-hover/link:text-[#00e5ff] transition-colors line-clamp-2">
                      {rev.video_title}
                    </h3>
                  </Link>
                  ...
                  {rev.notes && (
                    <p className="text-xs text-slate-800 dark:text-slate-200 italic bg-slate-50 dark:bg-[#070b14] p-2.5 rounded-lg border border-slate-200 dark:border-white/10 font-sans leading-relaxed">
                      "{rev.notes}"
                    </p>
                  )}
                </div>
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-white/[0.08] flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <Link
                      to={`/playlist/${rev.playlist_id}?videoId=${rev.video_id}`}
                      className="p-1.5 rounded-lg bg-cyan-100 dark:bg-[#00e5ff]/15 hover:bg-cyan-200 dark:hover:bg-[#00e5ff]/25 text-cyan-900 dark:text-[#00e5ff] border border-cyan-300 dark:border-[#00e5ff]/30 transition-colors"
                      title="Play video"
                    >
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-mono font-bold flex items-center gap-1 transition-all ${
                        isRevised
                          ? 'bg-slate-100 dark:bg-[#070b14] text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-white/15'
